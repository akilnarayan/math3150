...
%
% Differs from heat_equation.m in that the Fourier modes are not plotted.
...
set(ylabel('$\mathbf{u(x,0)}$'), labelprops{:});
set(gca, axesprops{:});
axis equal
...
while (T - t) > 0
  ...
  subplot(2,1,2);
  ax = axis();
  set(usol, 'ydata', phinx*(c.*exp(-k*lambda.^2*t)));
  set(utitle, 'string', ['$\mathbf{t=' num2str(t) '}$']);
  axis(ax);
  drawnow;
